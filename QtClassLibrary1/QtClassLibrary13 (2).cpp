...
//óñòàðåâøàÿ ôóíêöèÿ ïîñòðîåíèÿ ñòðóêòóð
/*
//???????? ??????? ??? ??????????? (????????)
...
//ôóíêöèÿ ïîñòðîåíèÿ ñòðóêòóðíûõ îïèñàíèé êàæäîãî ñëîæíîãî ýëåìåíòà
		...
		for (int k = 0; k < vEleMatr.size(); ++k)
		{
			for (int i = 1; i < restTable->rowCount(); ++i)
			{
				for (int j = 1; j < restTable->columnCount(); ++j)
				{
					...
					{
						zeroRowCount = 1; //íà÷àëüíîå ñìåùåíèå
						for (int zerorow = 1; zerorow < restTable->columnCount(); ++zerorow) //ðàñ÷åò äîï. ñìåùåíèÿ
						{
...
void MyModuleForm::IS_WidgetActived() //ìåñòíûé main
{
	currElement = 1;
...
//????? ? tableWidget (????????)
	...
	//âåêòîð âñåõ íåýëåìåíòàðíûõ äåéñòâèé
	for (int i = 0; i < cm->getDynamicStructure(0)->getElementsCount(); ++i)
	{
	...
	//ñîçäàíèå âåêòîðà êîäîâ âñåõ ýëåìåíòîâ
	for (int i = 0; i < vComplicated.size(); ++i)
	{
		elements3.push_back(vComplicated[i]->getChildren()); //ïîëó÷åíèå âñåõ "äåòåé" âñåõ ñëîæíûõ ïðåäìåòíûõ äåéñòâèé
		for (int j = 0; j < elements3[i].count(); ++j)
		{
			codes.push_back(QString::fromWCharArray(elements3[i].element<AFElement *>(j)->getCode().data())); //çàïèñü êîäîâ âñåõ äåòåé â âåêòîð
		}
		codesfull.push_back(codes); //ïîëó÷àåòñÿ âåêòîð âåêòîðîâ, ñîäåðæàùèé êîä ñëîæíîãî äåéñòâèÿ è êîäû âñåõ åãî äåòåé ñîîòâåòñòâåííî
		codes.clear(); //î÷èñòíà áóôåðíîãî âåêòîðà
	}

	//âñÿêèå óòèëèòàðíûå ïåðåìåííûå, êîòîðûå íåïëîõî áûëî áû ðàñêèäàòü ïî ôóíêöèÿì
	int w, h, maxw = 0, maxh = 0, wsum = 0, p = 0, l = 0, zeroC = 0, flag = 0, ccount = 0, tableS2Size = 0, tableSSize = 0, S2Span = 0, SSpan = 0;
	
	std::vector<int> maxwidth; //âåêòîð ðàçìåðà êîë-âà ñòðóêòóðíûõ îïèñàíèé (ìîæíî áûëî ñäåëàòü ïðîùå)
	for (int i = 0; i < 3; ++i)
		maxwidth.push_back(0);

	//âîò ýòîãî ÿ âîîáùå íå ïîìíþ....
	...
	//ñîçäàíèå òàáëèö â gridLayout-å
	for (int k = 0; k < vComplicated.size(); ++k)
	{
	...
	//èíèöèàëèçàöèÿ ðàñøèðåííîé îáùåé òàáëèöû
	if (vComplicated.size() > 0)
	{
		fullTableAdd1->clear();
    ...
    //èíèöèàëèçàöèÿ òàáëèöû ðåñòðóêòóðèçàöèè
	if (vComplicated.size() > 0)
	{
		restTable->clear();
...
*/
	...
	createSpecTables(); //ôóíêöèÿ ñîçäàíèÿ ñïåöèôèêàöèé
	

//çàïîëíåíèå ñïåöèôèêàöèè S1
	if (vComplicated.size() > 0)
	{
...
//ñòðàííûå íàðàáîòêè, â êîòîðûõ íè÷åãî íå ïîíèìàþ
	...
	//ôóíêöèÿ èòåðàöèîííîãî ðàñøèðåíèÿ îáùåé òàáëèöû (1 èòåðàöèÿ)
	...
	//ôóíêöèÿ èòåðàöèîííîãî ðàñøèðåíèÿ îáùåé òàáëèöû (âñå èòåðàöèè)
	...
	//ôóíêöèÿ ñîçäàíèÿ òàáëèö ñïåöèôèêàöèé
	void MyModuleForm::createSpecTables()
	{
		...
		S1HeaderModel->setData(S1HeaderModel->index(0, 0), "Код сложного действия", Qt::DisplayRole);
		S1HeaderModel->setData(S1HeaderModel->index(0, 2), trUtf8("Роль"), Qt::DisplayRole);
		S1HeaderModel->setData(S1HeaderModel->index(1, 0), QString::fromLocal8Bit("Óðîâåíü"), Qt::DisplayRole);
		S1HeaderModel->setData(S1HeaderModel->index(1, 1), QString::fromLocal8Bit("Ïîçèöèÿ"), Qt::DisplayRole);

		S2HeaderModel->setColumnCount(4);
		S2HeaderModel->setRowCount(2);
		S2HeaderModel->setData(S2HeaderModel->index(0, 0), QString::fromLocal8Bit("Êîä ñëîæíîãî äåéñòâèÿ"), Qt::DisplayRole);
		S2HeaderModel->setData(S2HeaderModel->index(0, 2), QString::fromLocal8Bit("Êîä äåéñòâèÿ"), Qt::DisplayRole);
		S2HeaderModel->setData(S2HeaderModel->index(1, 0), QString::fromLocal8Bit("Óðîâåíü"), Qt::DisplayRole);
		S2HeaderModel->setData(S2HeaderModel->index(1, 1), QString::fromLocal8Bit("Ïîçèöèÿ"), Qt::DisplayRole);
		S2HeaderModel->setData(S2HeaderModel->index(1, 2), QString::fromLocal8Bit("Óðîâåíü"), Qt::DisplayRole);
		S2HeaderModel->setData(S2HeaderModel->index(1, 3), QString::fromLocal8Bit("Ïîçèöèÿ"), Qt::DisplayRole);
