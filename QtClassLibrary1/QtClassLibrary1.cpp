...
/*глобальные модели*/
	...
	std::vector<iterationColoring>  iterationsV;
	...
	bool projectDataChanged;

//устаревшая функция построения структур
/*
//ñîçäàíèå ìàòðèöû äëÿ àëüòåðíàòèâ (óñòàðåëî)
...
//ñîçäàíèå ìàòðèöû äëÿ ïîñëåäîâàòåëüíîñòåé (óñòàðåëî)
...
//ñîçäàíèå ìàòðèöû äëÿ èòåðàöèé/öèêëîâ (óñòàðåëî)
...
{
	//auto ConceptualStructure = IS::getConceptModel()->getDynamicStructure(0);
	//auto ElementsVector = std::vector<SubjectRelation*>();
	std::vector<QString> ArgumentsVector;
	ArgumentsVector.clear();
	/*
	for (auto i = 0; i < ConceptualStructure->getElementsCount(); ++i)
	{
		ElementsVector.push_back((SubjectRelation*)ConceptualStructure->getAFElement(i));
	}
	...
	*/
	...
	for (auto SubjectRelationElement : ElementsVector)
	{
		...
		for (int i = 0; i < args1.count(); ++i)
		{
			...
			for (auto it = ElementsVector.begin(); *it != SubjectRelationElement; ++it)
			{
				if ((*it)->getType() != FEType::None) continue;
				if ((*it)->getFunction() == args1.element<AIElement*>(i))
				{
					//нашли
					ui.textBrowser->append(QString::fromWCharArray((*it)->getCode().data()) + " \n");
...
{	
	...
	if (element->getType() == FEType::Sequanse)
	{
		...
		vEleMatr[k].childMatrix->setData(vEleMatr[k].childMatrix->index(code.size(), 0), QColor(240, 100, 10), Qt::BackgroundColorRole);
	}
...
//		ui.tableView_3->setVisible(false);
		...
		auto ConceptualStructure = cm->getDynamicStructure(0);
		for (auto i = 0; i < ConceptualStructure->getElementsCount(); ++i)
		{
			ElementsVector.push_back((SubjectRelation*)ConceptualStructure->getAFElement(i));
		}
		std::sort(ElementsVector.begin(), ElementsVector.end(), AFElement::AFEcodeOrder);
		...
		for (int m = 0; m < elementsIterations.size(); ++m)
		{
			for (int l = 1; l < elementsIterations[m].count(); ++l)
			{
				...
				{
					iterationsV[m].children.push_back(QString::fromWCharArray(elementsIterations[m].element<AFElement *>(l)->getCode().data()));
		...
		int tableS2Size = 0, tableSSize = 0, S2Span = 0, SSpan = 0;
			...
			if (vComplicated.size() > 0)
			{
				...
				ui.tableView->setFixedHeight(vEleMatr[0].childMatrix->rowCount() * 24 - vEleMatr[0].childMatrix->rowCount()+2);
				ui.tableView->setFixedWidth(vEleMatr[0].childMatrix->columnCount() * 40 - vEleMatr[0].childMatrix->columnCount()+2);
			...
			if (vComplicated.size() > 0)
			{
				...
				ui.addLable->setText(vEleMatr[0].code + " = ");
				ui.addLable->setVisible(true);
				ui.tableView_3->setVisible(true);
				ui.tableView_3->verticalHeader()->hide();
				ui.tableView_3->horizontalHeader()->hide();
				ui.tableView_3->setFixedHeight(vEleMatr[0].childMatrix->rowCount() * 24 - vEleMatr[0].childMatrix->rowCount() + 2);
				ui.tableView_3->setFixedWidth(vEleMatr[0].childMatrix->columnCount() * 40 - vEleMatr[0].childMatrix->columnCount() + 2);
			...
			if (vComplicated.size() > 0)
			{
				...
				ui.restLable->setText(vEleMatr[0].code + " = ");
			}
			*/
...
//			ui.tabWidget->setTabEnabled(3, true);
	...
	void MyModuleForm::allSteps()
	{
		...
		if ((vEleMatr.size()) > 1 && (currElement < vEleMatr.size()))
		{
			...
			QProgressDialog dlg("Создание полной таблицы...", "Отмена", 0, (ElementsVector.size() + 7));			
			...
			dlg.setWindowIcon(QIcon("Screenshot_1.jpg"));
			...
			for (int i = 0; i < fullTable->rowCount(); ++i)
			{
				for (int j = 0; j < fullTable->columnCount(); ++j)
				{
					if (j - 1 == i)
					{
						...
						auto ArgumentsVector = OutInfromationLinkCheck(fullTable->index(i, j).data(Qt::DisplayRole).toString());
						if (ArgumentsVector.size() > 0)
						{
							std::vector<int> ArgumentCheck;
							ArgumentCheck.clear();
							...
							for (int k = 0; k < ArgumentsVector.size(); ++k)
							{
								for (int c = 0; c < fullTable->rowCount(); ++c)
								{
									for (int m = 1; m < fullTable->columnCount(); ++m)
									{
										if (m - 1 == c)
										{
											if (fullTable->index(c, m).data() == ArgumentsVector[k])
											{
												fullTable->setData(fullTable->index(m, j), "1", Qt::DisplayRole);
												ArgumentCheck[k] = 1;
											}
										}
									}
								}
								if (k == ArgumentsVector.size() - 1)
								{
									for (int c = 0; c < ArgumentsVector.size(); c++)
									{
										if (ArgumentCheck[c] == 0)
										{
											fullTable->setData(fullTable->index(fullTable->rowCount() - 1, j), "1", Qt::DisplayRole);
...
//			ui.tabWidget->setTabEnabled(3, true);
		...
		for (int m = 0; m < iterationsV.size(); m++)
		{
			for (int l = 0; l < iterationsV[m].children.size(); l++)
			{
				//if (elementsIterations[m].element<AFElement *>(l)->getType() == FEType::None)
					for (int i = 0; i < fullTable->rowCount(); i++)
						for (int j = 1; j < i+2; j++)
						{
							if (iterationsV[m].children[l] == fullTable->index(i, j).data(Qt::DisplayRole).toString())
							{
								fullTable->setData(fullTable->index(i, j), iterationsV[m].iterColor, Qt::BackgroundColorRole);
								fullTableAdd1->setData(fullTableAdd1->index(i, j), iterationsV[m].iterColor, Qt::BackgroundColorRole);
								if (l == 0)
								{
									iterationsV[m].startI = i;
									iterationsV[m].startJ = j;
								}
								if (l == iterationsV[m].children.size()-1)
								{
									iterationsV[m].finishI = i;
									iterationsV[m].finishJ = j;
								}
							}
						}
			}
		...
		for (int m = 0; m < iterationsV.size(); m++)
		{
			...
			//else
			{
				for (int i = iterationsV[m].startI; i <= iterationsV[m].finishI; i++)
				{
					for (int j = iterationsV[m].startJ; j <= iterationsV[m].finishJ; j++)
					{
						fullTable->setData(fullTable->index(i, j), iterationsV[m].iterColor, Qt::BackgroundColorRole);
						fullTableAdd1->setData(fullTableAdd1->index(i, j), iterationsV[m].iterColor, Qt::BackgroundColorRole);
					}
	...
	void MyModuleForm::createItemTables()
	{
		...
		QProgressDialog dlg("Создание таблиц...", "Отмена", 0, vComplicated.size()+2);
		dlg.setWindowTitle("Реструктуризация");
		dlg.setAutoClose(false);
		dlg.setMinimumDuration(1);
		dlg.setWindowModality(Qt::WindowModal);
